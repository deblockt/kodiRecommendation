...
public class SeasonAndEpisodeParser extends QueryParser {
    @Override
    protected QueryData internalParse(String query) {
        Pattern p = Pattern.compile("(.+)\\s*Saison\\s*(\\d+)\\s*[E|é]pisode\\s*(\\d+).*", Pattern.CASE_INSENSITIVE);
        ...
        if (b) {
            ...
            data.episode = Integer.parseInt(m.group(3));
        }
